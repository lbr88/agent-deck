...
- [x] **Step 1: Write the failing cross-provider regression**
...
- [x] **Step 3: Implement the shared invariant**
...
- [x] **Step 4: Verify GREEN and provider reconciliation behavior**
...
- [x] **Step 5: Update stale surface contracts**
...
- [x] **Step 6: Run focused and affected-package verification**

  Run:

  ```bash
  go test ./internal/session -run 'TestCreateForkedInstanceForTool|TestUpdateStatus.*ForkTitle' -count=1
  ...
  go test -race ./internal/session -run 'TestCreateForkedInstanceForTool|TestUpdateStatus.*ForkTitle' -count=1
