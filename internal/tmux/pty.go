...
func (s *Session) attachCmd(ctx context.Context) *exec.Cmd {
	return EnsureSaneAttachTERM(s.tmuxCmdContext(ctx, "-u", "attach-session", "-t", s.Name))
}

func (s *Session) attachReadOnlyCmd(ctx context.Context) *exec.Cmd {
	return EnsureSaneAttachTERM(s.tmuxCmdContext(ctx, "-u", "attach-session", "-r", "-t", s.Name))
}
