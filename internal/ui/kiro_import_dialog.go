...
type kiroImportSubmitMsg struct{}

// KiroImportDialog lets the user choose a saved Kiro CLI session from the Kiro
// session metadata directory.
type KiroImportDialog struct {
	visible      bool
	entries      []session.KiroSavedSession
	cursor       int
	selected     *session.KiroSavedSession
	width        int
	height       int
	searchActive bool
	searchQuery  string
...
func (d *KiroImportDialog) IsVisible() bool {
	return d.visible
}

func (d *KiroImportDialog) SetSize(width, height int) {
	d.width = width
	d.height = height
}
...
func (d *KiroImportDialog) Update(msg tea.Msg) (*KiroImportDialog, tea.Cmd) {
	...
	case "enter":
		...
		d.selected = &selected
		return d, func() tea.Msg { return kiroImportSubmitMsg{} }
	}
	return d, nil
}

func (d *KiroImportDialog) View() string {
	if !d.visible {
		return ""
	}
	...
	var lines []string
	lines = append(lines, fit(titleStyle.Render("Import Saved Kiro Session")))
	lines = append(lines, "")
	indexes := d.matchingIndexes()
	if len(d.entries) == 0 {
		lines = append(lines, fit(dimStyle.Render("No saved Kiro sessions found.")))
		...
		for pos := start; pos < end; pos++ {
			...
			if title == "" {
				title = shortKiroImportID(entry.ID)
			}
			updated := ""
...
func (d *KiroImportDialog) matchingIndexes() []int {
...
func shortKiroImportID(id string) string {
