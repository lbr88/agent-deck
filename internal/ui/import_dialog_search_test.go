...
func TestSavedSessionImportDialogsAdvertiseSlashSearch(t *testing.T) {
	...
	t.Run("kiro", func(t *testing.T) {
		...
		d.SetSize(120, 40)
		d.Show([]session.KiroSavedSession{{ID: "44444444-4444-4444-4444-444444444444", Title: "alpha", UpdatedAt: time.Now()}})
		if rendered := d.View(); !strings.Contains(rendered, "/ search") {
			t.Fatalf("import picker should advertise slash search:\n%s", rendered)
		}
	})
...
func TestSavedSessionImportDialogsSlashSearchFiltersAndSelects(t *testing.T) {
	...
	t.Run("kiro", func(t *testing.T) {
		...
		entries := []session.KiroSavedSession{
			{ID: "44444444-4444-4444-4444-444444444444", Title: "alpha", CWD: "/tmp/alpha", UpdatedAt: time.Now()},
			{ID: "55555555-5555-5555-5555-555555555555", Title: "fresh session", CWD: "/tmp/fresh", UpdatedAt: time.Now()},
		}
		d.Show(entries)
		typeSearch(d, "frsh")
		rendered := d.View()
...
func TestSavedSessionImportDialogsTypeToSearchWithoutSlash(t *testing.T) {
	t.Run("codex", func(t *testing.T) {
		d := NewCodexImportDialog()
		d.SetSize(120, 40)
		entries := []session.CodexIndexEntry{
		...
		d.Show(entries)
		typePlainSearch(d, "frsh")
		rendered := d.View()
		if strings.Contains(rendered, "alpha") {
			t.Fatalf("plain typing search should hide non-matching rows:\n%s", rendered)
		}
		if !strings.Contains(rendered, "fresh session") || !strings.Contains(rendered, "Search: frsh") {
			t.Fatalf("plain typing search should show matching row and query:\n%s", rendered)
		}
	})
...
func TestClaudeImportDialogSearchFindsPathTailMatch(t *testing.T) {
	d := NewClaudeImportDialog()
	d.SetSize(88, 40)
	now := time.Now()
	entries := []session.ClaudeImportCandidate{
		{
			...
			Name:      "granted-registry headless",
			CWD:       "/home/lrasmussen/git/domutech/granted-registry",
			UpdatedAt: now,
		},
		{
			SessionID: "22222222-2222-2222-2222-222222222222",
			Name:      "current work",
			CWD:       "/home/lrasmussen/git/private/agent-deck",
			UpdatedAt: now.Add(-time.Minute),
		},
	}
	d.Show(entries)
	typePlainSearch(d, "agent")
...
func TestOpenCodeImportDialogSearchFindsPathTailMatch(t *testing.T) {
	d := NewOpenCodeImportDialog()
	d.SetSize(88, 40)
	now := time.Now()
	entries := []session.OpenCodeImportEntry{
		{
			ID:        "ses_granted123",
			Title:     "granted-registry headless",
			Directory: "/home/lrasmussen/git/domutech/granted-registry",
			UpdatedAt: now,
		},
		{
			ID:        "ses_agent123",
			Title:     "current work",
			Directory: "/home/lrasmussen/git/private/agent-deck",
			UpdatedAt: now.Add(-time.Minute),
		},
	}
	d.Show(entries)
...
func TestKiroImportDialogSearchFindsPathTailMatch(t *testing.T) {
	d := NewKiroImportDialog()
	d.SetSize(88, 40)
	now := time.Now()
	entries := []session.KiroSavedSession{
...
func TestKiroImportDialogPathLinePreservesTailWhenNarrow(t *testing.T) {
	d := NewKiroImportDialog()
	d.SetSize(72, 40)
...
func typePlainSearch[D interface {
	Update(tea.Msg) (D, tea.Cmd)
}](d D, query string) {
	for _, r := range query {
		_, _ = d.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{r}})
