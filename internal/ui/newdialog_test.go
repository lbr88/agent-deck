...
func TestNewDialog_ModelSuggestions_FilterAndSelectCodex(t *testing.T) {
	...
	if len(d.modelSuggestions) != 4 || d.modelSuggestions[0] != "gpt-5.6-sol" || d.modelSuggestions[3] != "gpt-5.6" {
		t.Fatalf("filtered model suggestions = %v, want three GPT-5.6 tiers followed by the generic model", d.modelSuggestions)
	}
	d, _ = d.Update(tea.KeyMsg{Type: tea.KeyEnter})
	if !d.IsModelSuggestionsActive() {
		t.Fatal("enter on model input should activate the model suggestions dropdown")
...
func TestNewDialog_ModelDownMovesFocusUnlessPickerExplicitlyOpen(t *testing.T) {
	...
	if d.currentTarget() != focusReasoningEffort {
		t.Fatalf("currentTarget after down from model field = %v, want focusReasoningEffort", d.currentTarget())
	}
}
...
func TestDialogPresetCommands(t *testing.T) {
	...
	// Every built-in session tool appears in picker order.
	expectedCommands := []string{"", "claude", "gemini", "opencode", "codex", "kiro", "pi", "omp", "copilot", "crush", "cursor", "hermes", "deepseek"}
