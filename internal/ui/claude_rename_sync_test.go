...
func TestWebMutatorTitleUpdateSyncsClaudeMetadataAfterSave(t *testing.T) {
	...
	m := NewWebMutator(h)
	changed, _, warnings, err := m.UpdateSession("web-sync-001", map[string]string{
	...
	if len(warnings) != 0 {
		t.Fatalf("warnings = %q, want empty", warnings)
	}
...
func TestWebMutatorTitleUpdateReturnsClaudeRenameWarningWhenMetadataMissing(t *testing.T) {
	...
	m := NewWebMutator(h)
	changed, _, warnings, err := m.UpdateSession("web-sync-warning-001", map[string]string{
	...
	if len(warnings) != 1 || !strings.Contains(warnings[0], "Claude name sync failed") {
		t.Fatalf("warnings = %q, want Claude name sync warning", warnings)
	}
}
