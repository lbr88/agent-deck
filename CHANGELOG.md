...
- Support Oh My Pi (`omp`) as both a source and target in CLI and TUI session handovers, using the source entry's exact validated OMP conversation binding when available.
...
- **Oh My Pi (`omp`) is a first-class session tool.** Agent Deck detects OMP without confusing child Claude/Codex processes, isolates each conversation under `~/.omp/agent-deck/<instance-id>`, resumes and fresh-restarts sessions, copies persisted context when forking, tracks OMP busy/waiting/permission states, exposes OMP in the TUI/settings/setup wizard, supports `--model`, model-role routing, approval/profile/time controls, ephemeral sessions, and one-shot Claude/Codex imports.

### Changed

- **Merged upstream Agent Deck v1.13.0.** This release includes upstream remote-fleet, restart-persistence, MCP, notification, DeepSeek Harness, and Go 1.25.13 security updates.
...
- **Explicit Agent Deck Codex renames remain authoritative.** Native Codex thread titles no longer overwrite sessions whose title is locked.
