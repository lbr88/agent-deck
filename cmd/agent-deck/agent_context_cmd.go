...
func buildAgentHubContext(config *session.UserConfig) string {
	...
	b.WriteString("- agent-deck hub sessions attach|send|close|restart|restart-fresh|fork|rename|move|delete|archive|unarchive|remove|toggle-yolo|preview <node-name-or-id> <session-id-or-title> [...]\n")
