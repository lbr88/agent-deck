...
if GOTOOLCHAIN=go1.25.13 go test -run 'Issue699' ./internal/ui/ -count=1 -race >/tmp/adeck-699-gotest.log 2>&1; then
...
if [[ ! -x "$BIN" ]]; then
  ...
  GOTOOLCHAIN=go1.25.13 go build -o ./agent-deck ./cmd/agent-deck >/tmp/adeck-699-build.log 2>&1 || {
